...
def fmt_prop_pair(key: str, val) -> str:
    """
    Return "key = value" for use in SET clauses.
    Strings are quoted (with escaped single quotes), other types are JSON-dumped.
    """
...
def merge_node(label: str, name: str, props: Dict[str, str]) -> str:
    ...
    if props:
        # skip `name` key since it's in the MERGE match
        prop_pairs = [fmt_prop_pair(k, v) for k, v in props.items() if k != "name"]
        if prop_pairs:
            # each prop_pair is "key = value", so prefix with n.
...
def merge_relation(
    ...
    prop_set = ", ".join(f"r.{pair}" for pair in prop_pairs) if prop_pairs else ""
