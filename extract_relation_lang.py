...
system_msg = (
    "당신은 주어진 스키마에 따라 텍스트에서 **관계 인스턴스**(노드 간 관계)를 추출하여 "
    "RAG 시스템용 지식 그래프를 구축합니다. 반드시 하나의 JSON만 출력하세요."
)


def extract_relation_batch(
    texts: List[str],
    schema_json: Dict,
    purpose: str,
    batch_size: int,
...
### 목적
'{purpose}' RAG 시스템을 위한 관계 인스턴스를 추출하세요.

### 출력 형식 (추가 텍스트 금지)
{{
  \"nodes\": [],         # 필요한 경우 새 노드 포함 (스키마에 맞게)
  \"relations\": []     # 필수
}}

제약:
- 스키마에 명시된 label 및 property 타입만 사용
- start_node·end_node에는 반드시 노드 label 사용 (텍스트 그대로 X)
- 노드 name은 명확한 명사/명사구
- 일반 개념 노드 생성 금지
- 오직 1개의 JSON만 출력
...
"""
        ...
        # 4) Generate under autocast (fp16)
        with autocast():
...
def main(
    ...
    """extract_relation_lang.py 메인 진입점."""
    ...
    result_dir = os.path.join(OUTPUT_ROOT, "result_relation")
    ...
    merged_path = os.path.join(result_dir, "relation_result.json")
    ...
    for start in range(0, len(files), batch_size):
        ...
        for idx, parsed in zip(indices, batch_results):
            ...
            print(f"[{idx}] relation 추출 완료 (총 {len(batch_results)}개)")
